...
function Tree:get(prefix_i, index, plugin_context)
  local prefix_i_tbl = Util.parse_internal(prefix_i)
  local node = self.root
  index = index or #prefix_i_tbl
  if index < 0 then
    index = #prefix_i_tbl + index
  end
  for i = 1, index, 1 do
    node = node.children[prefix_i_tbl[i]]
