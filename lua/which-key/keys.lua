...
---@return Mapping[]
function M.parse_mappings(mappings, value, prefix_n, group_name)
  ...
  elseif type(value) == "table" then
    if #value == 0 then
      ...
      for k, v in pairs(value) do
        if k ~= "name" then
          M.parse_mappings(mappings, v, prefix_n .. k, value.name or nil)
        end
      end
      if prefix_n ~= "" then
        if value.name then
...
function M.map(mode, prefix_n, cmd, buf, opts)
  ...
  if buf ~= nil then
    ...
    vim.keymap.set(mode, prefix_n, cmd, opts)
  end
end
